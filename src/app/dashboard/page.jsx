...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        const res = await fetch("https://solace-2.onrender.com/api/dashboard/", {
    ...
    <div className="min-h-screen text-black bg-gray-100">
      ...
      <main className="container max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid sm:grid-cols-3 gap-6 mb-8">
          {[
            { icon: Smile, label: "Mood Check-ins", value: loading ? "..." : data.moodCheckins },
            { icon: MessageCircle, label: "Conversations", value: loading ? "..." : data.conversations },
            { icon: TrendingUp, label: "Wellness Score", value: loading ? "..." : data.wellnessScore },
          ].map((card, i) => (
            <div
              key={card.label}
              ...
              style={{ animation: `slide-up ${0.5 + i * 0.1}s ease-out forwards` }}
            >
              ...
              <p className="text-2xl font-display font-bold text-black mt-1">{card.value}</p>
            </div>
          ))}
        </div>

        {/* Quick Actions */}
        <h2 className="text-xl font-display font-bold text-black mb-4">Quick Actions</h2>
        <div className="grid sm:grid-cols-3 gap-6">
          {[
            { icon: MessageCircle, title: "Start a Chat", desc: "Talk to your AI companion about anything", href: "/chat" },
            { icon: UserCircle, title: "View Profile", desc: "Check your settings and preferences", href: "/profile" },
            { icon: Calendar, title: "Mood History", desc: "See how you've been feeling over time", href: "/dashboard" },
          ].map((action, i) => (
            <Link
              key={action.title}
              href={action.href}
              ...
              style={{ animation: `slide-up ${0.7 + i * 0.1}s ease-out forwards` }}
            >
              ...
              <h3 className="font-display font-bold text-black mb-1">{action.title}</h3>
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-2 text-black/70">
            ...
            <span className="text-sm italic">"You're doing great. One day at a time."</span>
          </div>
        </div>
      </main>
    </div>
